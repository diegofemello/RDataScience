pacotes <- c(
  "rgdal", "plotly", "tidyverse", "knitr", "kableExtra", "gridExtra",
...
shp_centro_oeste <- readOGR(
  dsn = "shapefile_centrooeste",
...
shp_centro_oeste_df <- tidy(shp_centro_oeste, region = "CD_GEOCODM") %>%
  ...
  mutate(
    state = substr(x = group, start = 1, stop = 2),
    state = factor(state,
...
# shp_centro_oeste_df:
shp_centro_oeste_df %>%
  mutate(capital = ifelse(city %in% c(
    "CAMPO GRANDE", "CUIABÁ",
    "GOIÂNIA", "BRASÍLIA"
  ),
  yes = TRUE,
...
shp_centro_oeste_df <- shp_centro_oeste_df %>%
  mutate(
    poverty_bands = ifelse(test = poverty <= 25.40,
      yes = "7.84 to 25.40",
